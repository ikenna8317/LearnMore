...
import { StudySet, AssessmentAttempt, GeneratedNote } from '../types';
import { Button, Card, Badge, Modal } from './UIComponents';
import { assessmentService } from '../services/assessment';
import { notesService } from '../services/notes';

interface SetAttemptsProps {
  userId: string;
  set: StudySet;
  onReviewAttempt: (attempt: AssessmentAttempt) => void;
}

export const SetAttempts: React.FC<SetAttemptsProps> = ({ userId, set, onReviewAttempt }) => {
  const [attempts, setAttempts] = useState<AssessmentAttempt[]>([]);
  const [loading, setLoading] = useState(true);
  const [relatedNotes, setRelatedNotes] = useState<Map<string, string>>(new Map()); 
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      const sourceNoteIds = set.items
        .filter(item => item.type === 'note')
        .map(item => item.id);
      
      const allRelevantNoteIds = [
        ...setNotes.map(n => n.id),
        ...sourceNoteIds
      ];

      const titleMap = new Map<string, string>();
      setNotes.forEach(n => titleMap.set(n.id, n.title || 'Untitled Set Note'));
      
      if (sourceNoteIds.length > 0) {
          const allUserNotes = await notesService.getAllNotes(userId); 
          allUserNotes.forEach(n => {
              if (sourceNoteIds.includes(n.id)) {
                  titleMap.set(n.id, n.title || 'Untitled Note');
              }
          });
      }
      setRelatedNotes(titleMap);

      let noteAttempts: AssessmentAttempt[] = [];
      if (allRelevantNoteIds.length > 0) {
        noteAttempts = assessmentService.getAttemptsByNoteIds(userId, allRelevantNoteIds);
      }
      
      const directSetAttempts = assessmentService.getAttemptsBySetId(userId, set.id);
      const allAttempts = [...noteAttempts, ...directSetAttempts]
        .sort((a, b) => b.timestamp - a.timestamp);
      
      const uniqueAttempts = Array.from(new Map(allAttempts.map(a => [a.id, a])).values());

      setAttempts(uniqueAttempts);

    } catch (e) {
      console.error("Failed to load set history", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
        await assessmentService.deleteAttempt(userId, deleteModal.attempt.id);
  ...
  if (loading) return <div className="text-center py-12 text-slate-400 dark:text-slate-500">Loading history...</div>;
  ...
  if (attempts.length === 0) {
    return (
        <div className="p-12 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-center bg-slate-50/50 dark:bg-slate-800/50">
            ...
            <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">No past attempts found</h3>
            <p className="text-slate-500 dark:text-slate-400 mb-6 max-w-md mx-auto">
                Quizzes, tests, and exams taken on notes or snapshots in this set will appear here.
            </p>
        </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-2">
         <h2 className="text-xl font-bold text-slate-900 dark:text-white">Assessment History</h2>
         <Badge color="slate">{attempts.length} Attempts</Badge>
      </div>
      ...
      <div className="grid gap-4">
        {attempts.map(attempt => {
            ...
            const noteTitle = attempt.noteId ? relatedNotes.get(attempt.noteId) : undefined;
            const sourceCount = attempt.sourceIds?.length;
            const contextLabel = noteTitle 
                ? <><span className="text-slate-400 dark:text-slate-500">Based on:</span> <span className="font-medium text-slate-600 dark:text-slate-300">{noteTitle}</span></>
                : sourceCount 
                    ? <><span className="text-slate-400 dark:text-slate-500">Snapshot:</span> <span className="font-medium text-slate-600 dark:text-slate-300">{sourceCount} Sources</span></>
                    : <span className="text-slate-400 dark:text-slate-500">Set Assessment</span>;
